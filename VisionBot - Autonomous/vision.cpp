...
// also compatible with masked processing
{
	...
	// copy a into b first
	 // size = a.width * a.height; // 640 x 480 > 65000 -> error
	// 4 byte  =  2 byte * 2 byte = 2 bytes -> overflow
	// 2 bytes only go up to 65000
	size = (i4byte)a.width * a.height; // 640 x 480 > 65000 
	// = the (i4byte) is a cast which converts width to 4 bytes 
...
// in the horizontal direction and jp is in the vertical direction.
{
	...
	double mi,mj,m,rho; // so overflow doesn't occur
	...
	for(j=0;j<height;j++) { // y-dir
		for(i=0;i<width;i++) { // x-dir
			if( pl[j*width+i] == nlabel ) {
				rho = pa[j*width+i]; // mass for i,j -> A = 1
				m  += rho; // sum of the mass
